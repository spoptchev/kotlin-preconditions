...
class MapMatcherTest {
    ...
    @Test fun `test haveKey valid`() {
        val result = matcher.haveKey(1).test(mapOf(1 to "one"))

        assertTrue(result.valid)
    }

    @Test fun `test haveKey invalid`() {
        val result = matcher.haveKey(1).test(mapOf(2 to "one"))
    ...
    @Test fun `test haveValue valid`() {
        val result = matcher.haveValue(1).test(mapOf("one" to 1))

        assertTrue(result.valid)
    }

    @Test fun `test haveValue invalid`() {
        val result = matcher.haveValue(1).test(mapOf("one" to 2))
    ...
    @Test fun `test contain valid`() {
        val result = matcher.contain(1, "one").test(mapOf(1 to "one"))

        assertTrue(result.valid)
    }

    @Test fun `test contain invalid`() {
        val result = matcher.contain(1, "two").test(mapOf(1 to "one"))
