...
interface CollectionMatcher {
    ...
    fun <T> hasSize(size: Int) = object : Matcher<Collection<T>>() {
    ...
    fun <T> contains(element: T) = object : Matcher<Collection<T>>() {
        override fun test(condition: Condition<Collection<T>>) = condition.test {
            withResult(value.contains(element)) { "$expectedTo contain element $element" }
        }
    }

    fun <T> isEmpty() = object : Matcher<Collection<T>>() {
        override fun test(condition: Condition<Collection<T>>) = condition.test {
            withResult(value.isEmpty()) { "$expectedTo be empty" }
        }
    }

    fun <T> containsAll(vararg values: T) = containsAll(values.asList())
    fun <T> containsAll(values: Collection<T>) = object : Matcher<Collection<T>>() {
        override fun test(condition: Condition<Collection<T>>) = condition.test {
            withResult(values.all(value::contains)) { "$expectedTo contain all values of $values" }
        }
    }

    fun <T : Comparable<T>> isSorted() = object : Matcher<Collection<T>>() {
        override fun test(condition: Condition<Collection<T>>) = condition.test {
